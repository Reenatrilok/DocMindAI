    ...
    <div style={s.root}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .main-btn:hover { transform: translateY(-2px); box-shadow: 0 14px 40px rgba(229,57,53,.45) !important; }
      ...
      <div style={s.layout}>
          ...
          <nav style={s.nav}>
            {NAV.map((item, i) => {
              ...
              return (
                ...
                >
                  <span style={{ fontSize: 15, width: 18, textAlign: "center", color: isActive ? "#e53935" : "rgba(255,255,255,.32)", transition: "color .2s" }}>{item.icon}</span>
        ...
        <main ref={mainRef} style={s.main}>
          ...
          <header style={{ ...s.header, opacity: mounted ? 1 : 0, animation: mounted ? "fadeUp .5s .1s both" : "none" }}>
            <div>
              <div style={s.headerSub}>Monday, June 08 · Welcome back</div>
          ...
          <div
            ...
            style={{
              ...s.uploadZone,
              ...(dragging ? { borderColor: "rgba(229,57,53,.55)", background: "rgba(229,57,53,.04)" } : {}),
              ...(file ? { borderColor: "rgba(16,185,129,.4)", borderStyle: "solid", background: "rgba(16,185,129,.03)" } : {}),
              opacity: mounted ? 1 : 0,
          ...
          >
            ...
            {["tl","tr","bl","br"].map(c => <div key={c} style={{ ...s.corner, ...s[`c_${c}`] }} />)}
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, textAlign: "center" }}>
              <div style={{ ...s.uploadIconWrap, background: file ? "rgba(16,185,129,.1)" : dragging ? "rgba(229,57,53,.1)" : "rgba(255,255,255,.04)", borderColor: file ? "rgba(16,185,129,.28)" : dragging ? "rgba(229,57,53,.35)" : "rgba(255,255,255,.09)" }}>
                <span style={{ fontSize: 26 }}>{file ? "✓" : "⬆"}</span>
              </div>
              <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 20 }}>{file ? file.name : "Drop your PDF here"}</div>
              <div style={{ fontSize: 13, color: "rgba(255,255,255,.33)" }}>
                {file ? `${(file.size / 1024 / 1024).toFixed(2)} MB — ready to process` : "or click to browse · PDF files supported"}
              </div>
              {file && (
                <button className="main-btn" style={s.processBtn}>✦ &nbsp;Process with AI</button>
              )}
            </div>
          </div>
          ...
          <section style={{ opacity: mounted ? 1 : 0, animation: mounted ? "fadeUp .5s .3s both" : "none" }}>
            ...
            <div style={s.toolsGrid}>
              {TOOLS.map((t, i) => (
                <div key={t.label} className="tool-card" style={{ ...s.toolCard, animation: mounted ? `fadeUp .5s ${.35 + i * .05}s both` : "none", transition: "transform .25s, border-color .25s" }}>
                  <div className="tool-icon-inner" style={{ ...s.toolIcon, background: `${t.color}18`, color: t.color, transition: "transform .25s" }}>{t.icon}</div>
          ...
          {/* Recent Docs */}
          <section style={{ opacity: mounted ? 1 : 0, animation: mounted ? "fadeUp .5s .5s both" : "none", marginTop: 36 }}>
            <div style={s.sectionHeader}>
            ...
            <div style={s.docTable}>
              ...
              {RECENT.map((doc, i) => (
                <div key={doc.name} className="doc-row" style={{ ...s.docRow, animation: mounted ? `fadeIn .4s ${.55 + i * .06}s both` : "none", transition: "background .2s" }}>
                  <div style={{ ...s.docCell, flex: 2, gap: 0 }}>
                    <div style={s.docThumb}><span style={{ fontSize: 9, fontWeight: 800, color: "#e53935", letterSpacing: .5 }}>PDF</span></div>
                    <span style={{ fontSize: 13.5, fontWeight: 500, marginLeft: 12, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{doc.name}</span>
                  </div>
                  <div style={s.docCell}>
                    <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 9px", borderRadius: 6, border: "1px solid", letterSpacing: .5, textTransform: "uppercase", background: `${doc.color}18`, color: doc.color, borderColor: `${doc.color}30` }}>{doc.tag}</span>
                  </div>
...
const s = {
  ...
  logoRing: { position: "absolute", inset: -3, border: "1px solid rgba(229,57,53,.28)", borderRadius: 14, animation: "pulse 3s infinite" },
  ...
  searchWrap: { display: "flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 10, padding: "9px 12px" },
  ...
  navBtn: { display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 10, background: "transparent", border: "1px solid transparent", color: "rgba(255,255,255,.42)", fontSize: 13.5, cursor: "pointer", textAlign: "left", fontFamily: "'Outfit',sans-serif", fontWeight: 500, width: "100%", transition: "all .2s", position: "relative" },
  navBtnActive: { background: "rgba(229,57,53,.1)", color: "#fff", border: "1px solid rgba(229,57,53,.2)" },
  activePip: { width: 4, height: 4, borderRadius: "50%", background: "#e53935", boxShadow: "0 0 6px #e53935" },
  upgradeCard: { background: "linear-gradient(135deg, rgba(229,57,53,.09), rgba(183,28,28,.05))", border: "1px solid rgba(229,57,53,.2)", borderRadius: 14, padding: "18px 16px", position: "relative", overflow: "hidden", animation: "borderGlow 4s infinite" },
  ...
  upgradeBtn: { background: "rgba(229,57,53,.14)", border: "1px solid rgba(229,57,53,.32)", borderRadius: 8, color: "#e53935", fontSize: 12, fontWeight: 600, padding: "7px 14px", cursor: "pointer", fontFamily: "'Outfit',sans-serif" },
  ...
  pill: { display: "flex", alignItems: "center", gap: 6, background: "rgba(16,185,129,.07)", border: "1px solid rgba(16,185,129,.18)", borderRadius: 20, padding: "6px 14px", fontSize: 12, color: "#6ee7b7", fontWeight: 500 },
  pillDot: { width: 6, height: 6, borderRadius: "50%", background: "#10b981", boxShadow: "0 0 6px #10b981", animation: "pulse 2s infinite" },
  notifBtn: { width: 38, height: 38, borderRadius: 10, background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.07)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 15 },
  ...
  uploadZone: { border: "1.5px dashed rgba(255,255,255,.1)", borderRadius: 22, padding: "52px 32px", cursor: "pointer", marginBottom: 36, position: "relative", background: "rgba(255,255,255,.015)", transition: "border-color .3s, background .3s" },
  corner: { position: "absolute", width: 14, height: 14, pointerEvents: "none" },
  c_tl: { top: 10, left: 10, borderTop: "2px solid rgba(229,57,53,.38)", borderLeft: "2px solid rgba(229,57,53,.38)", borderRadius: "4px 0 0 0" },
  c_tr: { top: 10, right: 10, borderTop: "2px solid rgba(229,57,53,.38)", borderRight: "2px solid rgba(229,57,53,.38)", borderRadius: "0 4px 0 0" },
  c_bl: { bottom: 10, left: 10, borderBottom: "2px solid rgba(229,57,53,.38)", borderLeft: "2px solid rgba(229,57,53,.38)", borderRadius: "0 0 0 4px" },
  c_br: { bottom: 10, right: 10, borderBottom: "2px solid rgba(229,57,53,.38)", borderRight: "2px solid rgba(229,57,53,.38)", borderRadius: "0 0 4px 0" },
  uploadIconWrap: { width: 64, height: 64, borderRadius: 18, border: "1px solid", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 4, transition: "all .3s" },
  processBtn: { marginTop: 8, background: "linear-gradient(135deg,#e53935,#c62828)", border: "none", borderRadius: 12, color: "#fff", fontSize: 14, fontWeight: 600, padding: "12px 28px", cursor: "pointer", fontFamily: "'Outfit',sans-serif", letterSpacing: .3, boxShadow: "0 8px 28px rgba(229,57,53,.32)", transition: "transform .2s, box-shadow .2s" },
  ...
  toolCard: { background: "rgba(255,255,255,.025)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 16, padding: "20px 18px", cursor: "pointer" },
  ...
  docTable: { background: "rgba(255,255,255,.02)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 16, overflow: "hidden" },
  docHead: { display: "flex", alignItems: "center", gap: 16, padding: "11px 20px", borderBottom: "1px solid rgba(255,255,255,.05)", fontSize: 10, letterSpacing: 1.5, color: "rgba(255,255,255,.22)", fontWeight: 600, textTransform: "uppercase" },
  docRow: { display: "flex", alignItems: "center", gap: 16, padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,.04)", cursor: "pointer" },
  docCell: { display: "flex", alignItems: "center", flex: 1, minWidth: 0 },
  docThumb: { width: 36, height: 36, borderRadius: 9, background: "rgba(229,57,53,.08)", border: "1px solid rgba(229,57,53,.16)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  actionBtn: { background: "rgba(229,57,53,.1)", border: "1px solid rgba(229,57,53,.2)", borderRadius: 7, color: "#e53935", fontSize: 11, fontWeight: 600, padding: "5px 10px", cursor: "pointer", fontFamily: "'Outfit',sans-serif", letterSpacing: .3 },
};